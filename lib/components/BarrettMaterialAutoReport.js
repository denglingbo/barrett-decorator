...
function tastRunner() {
    taskQueue.forEach(function (task) {
        var targets = task.fn();
        targets.forEach(function ($target) {
            var data = JSON.parse($target.getAttribute('bt-data'));
            ReportMaterial('view_material', __assign({}, task.params, data));
        });
    });
}
var timer = null;
                    ...
                    switch (_a.label) {
                        ...
                        case 1:
                            ...
                            configArr.forEach(function (conf, index) {
                                ...
                                if (!clickLocked) {
                                    delegate(document.body, 'click', targetSelector, function (e) {
                                        var dataStr = this.getAttribute('bt-data');
                                        if (!dataStr || dataStr === '[object Object]') {
                                            return;
                                        }
                                        try {
                                            var data = JSON.parse(dataStr);
                                            var event_1 = this.getAttribute('bt-event') || 'click_material';
                                            ReportMaterial(event_1, __assign({}, data, config.params));
                                        }
                                        catch (ex) {
                                        }
                                    });
                                    ...
                                    if (scrollSelector) {
                                        ...
                                        $targets.forEach(function ($target) {
                                            ...
                                            try {
                                                var data = JSON.parse($target.getAttribute('bt-data'));
                                                if (isReport) {
                                                    ReportMaterial('view_material', __assign({}, ret, config.params, data));
                                                }
                                            }
                                            catch (ex) {
                                            }
